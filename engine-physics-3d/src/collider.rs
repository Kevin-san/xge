...
use crate::{AABB, Axis, CollisionGroups, CombineRule, MassProperties};
...
impl ColliderShape3D {
    ...
    pub fn capsule(half_height: f32, radius: f32, axis: Axis) -> Self {
        Self::Capsule { half_height, radius, axis }
    }
    ...
    pub fn halfspace(outward_normal: Vec3) -> Self {
        Self::Halfspace { outward_normal: outward_normal.normalize() }
    }

    /// 计算AABB
    pub fn compute_aabb(&self, position: Vec3, rotation: Quat) -> AABB {
        match self {
            Self::Ball { radius } => AABB::from_center_half_extents(
                position,
                Vec3::splat(*radius),
            ),
            Self::Cuboid { hx, hy, hz } => {
            ...
            Self::Triangle { a, b, c } => {
                let min = position + Vec3::new(
                    a.x.min(b.x).min(c.x),
                    a.y.min(b.y).min(c.y),
                    a.z.min(b.z).min(c.z),
                );
                let max = position + Vec3::new(
                    a.x.max(b.x).max(c.x),
                    a.y.max(b.y).max(c.y),
                    a.z.max(b.z).max(c.z),
                );
    ...
    fn compute_rotated_half_extents(half_extents: Vec3, rotation: Quat) -> Vec3 {
        ...
        // 计算旋转矩阵的绝对值行
        let row0 = Vec3::new(
            abs_rot.w * abs_rot.w + abs_rot.x * abs_rot.x - abs_rot.y * abs_rot.y - abs_rot.z * abs_rot.z,
    ...
    pub fn compute_mass(&self, density: f32) -> f32 {
        match self {
            Self::Ball { radius } => {
                (4.0 / 3.0) * PI * radius * radius * radius * density
            },
            Self::Cuboid { hx, hy, hz } => {
                8.0 * hx * hy * hz * density
            },
            ...
            Self::ConvexHull { points } => {
                Self::compute_convex_hull_volume(points) * density
            },
    ...
    fn compute_inertia(&self, mass: f32) -> Vec3 {
        match self {
            ...
            Self::Capsule { half_height, radius, .. } => {
                let i_cylinder = mass * (3.0 * radius * radius + 4.0 * half_height * half_height) / 12.0;
                Vec3::new(i_cylinder, i_cylinder, i_cylinder)
            },
            Self::Cylinder { half_height, radius } => {
                let i_radial = mass * (3.0 * radius * radius + 4.0 * half_height * half_height) / 12.0;
...
impl Collider3D {
    ...
    pub fn set_sensor(&mut self, sensor: bool) {
        self.collider_type = if sensor { ColliderType::Sensor } else { ColliderType::Solid };
    }
...
impl Collider3DBuilder {
    ...
    pub fn mass_properties(mut self, mp: MassProperties) -> Self {
        self.collider.set_density(mp.mass / self.collider.shape.compute_mass(1.0).max(0.001));
        self
    }
